...
var PlatformErrors = require('./Utilities/PlatformErrors');
injector.service('Errors', PlatformErrors);
...
function Pomegranate(){
    ...
    if(initialized && instanceOptions) {
      ...
      Controllers.on('ready', function() {
        ...
        Deps.on('ready', function(){
          ...
          /**
           * Load user defined global handlers
           */
          _.each(useFunctions, function(useFn, key){
            useFn(loadedApp)
          })
          /**
  ...
  /**
   *  Provides a reference to the core express app object to attach global
   *  request handlers to.
   *
   * @param {Function} useFn
   * @returns {Pomegranate}
   */
  this.useGlobal = function(useFn){
    if(!started && initialized) {
      useFunctions.push(useFn)
      return this
    }
    this.error(new Error('Cannot add global handlers after server start.'));
    return this
  };
  /**
...
process.on('uncaughtException', function(err){
  ...
  console.log(chalk.red('Pomegranate has encountered an uncaught exception and will now exit.'))
})
