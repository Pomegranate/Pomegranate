...
function Pomegranate(){
  ...
  var attemptStart = function(cb) {
    if(maxStartAttemts >= 1){
      if(_.every(dataServices, 'ready', true)){
        ...
        this.log(chalk.inverse.green("Pomegranate: Server starting on " + instanceOptions.address + ':' + instanceOptions.port));
  ...
  this.init = function(options) {
    ...
    initialized = true;
    this.log(chalk.inverse.green("Pomegranate: Running setup."));
