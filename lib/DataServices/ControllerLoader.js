...
module.exports = function(injector, options, logger) {
  ...
  logger.log(chalk.green('ControllerLoader: Loading Controllers from ' + controllerDir));
  fs.readdir(controllerDir, function(err, files) {
    var count = 0

    if(err){
      if(err.code === 'ENOENT'){
        logger.log(chalk.yellow('ControllerLoader: No controllers loaded.'));
    ...
    files
      ...
      .forEach(function(file) {
        ...
        logger.log(chalk.green('ControllerLoader: Loaded ' + fileName + ' controller.'));
      })
    if(count > 0) {
      logger.log(chalk.green('ControllerLoader: loaded ' + count + ' controllers.'));
    }
