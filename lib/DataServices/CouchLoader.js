...
module.exports = function(injector, options, logger) {
  ...
  logger.log(chalk.green('CouchLoader: Loading CouchDB models from ' + modelDir));
  ...
  fs.readdirSync(modelDir)
    ...
    .forEach(function(file) {
      ...
      logger.log(chalk.green('CouchLoader: Loaded model - ' + modelName));
      ...
      builtDbs.push(
        CouchAsync.getAsync(modelName)
          .spread(function(body) {
            ...
            if (!opts.persist) {
              logger.log(chalk.yellow('CouchLoader: Destroying ' + modelName + ' DB file.'));
  ...
  function afterCreate(db, model, name) {
    logger.log(chalk.green('CouchLoader: Created ' + name + ' DB file.'));
    var views = [];
