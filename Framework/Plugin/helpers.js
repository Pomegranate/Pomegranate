...
exports.getName = fp_1.get('computedMetadata.name');
exports.getDirectories = fp_1.get('state.directories');
exports.getParents = fp_1.get('loadMetadata.parents');
exports.getNamespace = fp_1.get('loadMetadata.namespace');
exports.hasParents = (parents) => {
    return !!(parents.length);
};
exports.getFqn = fp_1.get('computedMetadata.fqn');
...
exports.configObjectPath = fp_1.curry((plugin, appendPath) => {
    ...
    return ArrayMethods_1.append((exports.hasParents(parents) ? [...fp_1.tail(parents), fp_1.last(plugin.computedMetadata.name)] : []), [appendPath]).join('.');
});
exports.configPath = (plugin) => {
    let parents = exports.getParents(plugin);
    return (exports.hasParents(parents) ? [...fp_1.tail(parents), fp_1.last(plugin.state.configuration.name)] : []);
};
exports.getConfigFilePath = (plugin) => {
    let Parents = fp_1.get('loadMetadata.parents', plugin);
    let BaseFile = Parents.length ? fp_1.first(Parents) : exports.fqDeclaredName(plugin.computedMetadata.name);
    let Namespace = fp_1.get('loadMetadata.namespace', plugin);
