...
export const updateFrameworkMeta = (LogManager: LogManager, FrameworkConfiguration: ValidatedTransformer, frameworkMetrics: Metrics, skeletons: ValidatedPlugin[]) => {
  frameworkMetrics.startFrameworkPhase('RuntimeState')

  let injectableParameters = injectableParams(skeletons)
  let availablePlugins = map(getFqShortname, skeletons)
  let providingPlugins = getProvidingPlugins(skeletons)
  let allAvailable = concat(injectableParameters, availablePlugins)
